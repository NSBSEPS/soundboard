...
export default async function EstimateDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: catalog } = await supabase
    ...
    .select("id, category, name, default_price, default_location_type, default_requires_client_visit, default_depends_on_previous, typical_duration_days")
    .eq("active", true)
    .order("category")
    .order("sort_order");
  ...
  return (
    <main style={{ maxWidth: 640, margin: "0 auto", padding: 24 }}>
      ...
      {estimate.status === "draft" && (
        <AddPhaseForm estimateId={estimate.id} catalog={catalog ?? []} />
      )}
