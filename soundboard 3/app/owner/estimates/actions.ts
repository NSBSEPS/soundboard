...
export async function addLineItem(estimateId: string, formData: FormData) {
  ...
  // whether to push this price back as the new standard, and — for a
  // brand-new task typed from scratch — whether to save it as a reusable
  // catalog entry instead of a one-off. This is the actual fix for having
  // to retype the same task and price on every estimate: pick it once,
  // correct the price if it's changed, and it's the standard from then on.
  ...
  // Keep the catalog's standard price current, if asked. This runs after
  // the line item insert succeeds, and deliberately doesn't fail the whole
  // action if it errors — the estimate itself is what matters most here;
  // a catalog sync issue shouldn't block adding the line item.
  if (catalogId && updateCatalogPrice) {
    const { error: catalogError } = await supabase.from("service_catalog").update({ default_price: amount }).eq("id", catalogId);
    if (catalogError) console.error("Failed to update catalog price:", catalogError.message);
  } else if (!catalogId && saveAsNewService && newServiceCategory) {
    const { data: maxSort } = await supabase
      ...
      .order("sort_order", { ascending: false })
      .limit(1);
    const { error: catalogError } = await supabase.from("service_catalog").insert({
    ...
    if (catalogError) console.error("Failed to save new catalog service:", catalogError.message);
  }
